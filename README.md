...
// Рассмотрим такие квалификаторы как: const, extern, static, restrict, volatile, auto, register.

// Пример использования const
const int a = 123; // Переменная a является константной и не может быть изменена
// a = 555; // Ошибка: попытка изменить константу

// Указатель на константу
const int b = 456;
const int *p = &b; // Указатель на константу
// *p = 789; // Ошибка: попытка изменить значение через указатель на константу
p = (int *)5; // Ок: можно изменить адрес указателя

// Константный указатель
int c = 10;
int *const p2 = &c; // Константный указатель
*p2 = 20; // Ок: можно изменить значение переменной через константный указатель
// p2 = (int *)5; // Ошибка: нельзя изменить адрес константного указателя

// Константный указатель на константу
const int d = 30;
const int *const p3 = &d; // Константный указатель на константу
// *p3 = 40; // Ошибка: нельзя изменить значение через константный указатель на константу
// p3 = (int *)5; // Ошибка: нельзя изменить адрес константного указателя

// Пример использования extern
// file1.c
int a_global = 10; // Глобальная переменная

// file2.c
extern int a_global; // Объявление глобальной переменной из другого файла
void func() {
    a_global = 20; // Использование глобальной переменной
}

// Пример использования static
static int func() { // Статическую функцию можнно исользовать только в текущем файле 
    static int a = 0; // Статическая переменная, инициализируется один раз
    a++;
    return a;
}
...
// Пример использования restrict(нужен для оптимизации если масивы не пересекаются в памяти)
...
int main() {
    while (flag == 0) {
        // Код, который может быть прерван изменением flag
    }
    return 0;
}

// Пример использования auto (в C это локальная переменная)[устарело]
auto int x = 10; // Эквивалентно int x = 10;
...
- 📄[Доп. задание под*: Рюкзак](https://eios.sibsutis.ru/mod/resource/view.php?id=168175)
- 📄[Лекция отладчик GDB](https://github.com/kruffka/C-Programming/blob/master/2024-2025/slides_pptx/8_GDB.md)
- 📄[Лекция Valgrind и ASan](https://github.com/kruffka/C-Programming/blob/master/2024-2025/slides_pptx/8.1_valgrind_sanitizer.md)
- 📄[Лекция. Многофайловые проекты. Make, CMake](https://github.com/kruffka/C-Programming/blob/master/2024-2025/slides_pptx/9_make_cmake.md)
